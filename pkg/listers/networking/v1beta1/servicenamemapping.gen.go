...
type ServiceNameMappingLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.ServiceNameMapping, err error)
...
// List lists all ServiceNameMappings in the indexer.
func (s *serviceNameMappingLister) List(selector labels.Selector) (ret []*v1beta1.ServiceNameMapping, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.ServiceNameMapping))
	})
	return ret, err
}
...
type ServiceNameMappingNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.ServiceNameMapping, error)
...
// List lists all ServiceNameMappings in the indexer for a given namespace.
func (s serviceNameMappingNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.ServiceNameMapping, err error) {
...
// Get retrieves the ServiceNameMapping from the indexer for a given namespace and name.
func (s serviceNameMappingNamespaceLister) Get(name string) (*v1beta1.ServiceNameMapping, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("servicenamemapping"), name)
	}
